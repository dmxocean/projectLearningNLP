# -*- coding: utf-8 -*-
"""
Health Fake News Detection - RoBERTa + Summarization Evidence
==============================================================
Solves RoBERTa's 512-token limit by summarising main_text with
facebook/bart-large-cnn, then feeding the summary to RoBERTa.

Pipeline:
  claim + explanation + bart_summary  ->  RoBERTa-base  ->  4-class label
...
# --- SUMMARIZER (BART) -------------------------------------------------------
# facebook/bart-large-cnn is loaded once on CPU/GPU at startup.
# It compresses main_text (often thousands of tokens) into ~130 tokens,
# which fits cleanly inside RoBERTa's remaining budget after claim+explanation.
#
# Why BART and not extractive TF-IDF?
#   - Generates a coherent compressed representation, not a bag of sentences
#   - Handles paraphrase and inference that TF-IDF cannot capture
#   - facebook/bart-large-cnn is pre-trained on CNN/DailyMail news articles --
#     domain-appropriate for the journalistic PubHealth source texts
#
# Token budget breakdown (512 total):
#   <s> claim </s></s> explanation </s></s> summary </s>
#   ~2  ~60      2       ~100         2      ~130-150   1  = ~297-320 (well within limit)

SUMMARIZER_MODEL = "facebook/bart-large-cnn"
SUMMARY_MIN_LEN  = 30    # always generate at least this many tokens

print("Loading BART summarizer (facebook/bart-large-cnn)...")
_summ_device   = torch.device("cuda" if torch.cuda.is_available() else "cpu")
_summ_tokenizer = AutoTokenizer.from_pretrained(SUMMARIZER_MODEL)
_summ_model     = AutoModelForSeq2SeqLM.from_pretrained(SUMMARIZER_MODEL).to(_summ_device)
_summ_model.eval()
print(f"  Summarizer ready on {_summ_device}.")
...
def summarize_text(text: str, max_new_tokens: int = 200) -> str:
    """
    Summarise main_text with BART.

    max_new_tokens is computed by build_input() from the actual remaining
    token budget after encoding claim + explanation, so the summary fills
    almost the entire 512-token window rather than a fixed length.

    Texts under 60 words are returned as-is (nothing meaningful to compress).
    """
    ...
    if len(words) < 60:
        return text.strip()
    # BART's own context window is 1024 tokens; truncate input to be safe
    try:
        ...
        with torch.no_grad():
            summary_ids = _summ_model.generate(
                **inputs,
                ...
                min_length=min(SUMMARY_MIN_LEN, max_new_tokens - 1),
                num_beams=4,
                no_repeat_ngram_size=3,
                early_stopping=True,
            )
        return _summ_tokenizer.decode(
            summary_ids[0], skip_special_tokens=True
        ).strip()
    except Exception:
        # Fallback: first max_new_tokens whitespace-tokens if BART fails
        return " ".join(words[:max_new_tokens])
...
def build_input(claim: str, explanation: str, main_text: str):
    ...
    Encoding:
      <s> claim </s></s> explanation </s></s> bart_summary </s>

    Budget calculation
    ------------------
    1. Tokenise claim + explanation to measure how many tokens they consume.
    2. Subtract from MAX_TOKENS (512) to get the remaining budget.
    3. Reserve 5 tokens for special tokens and separators (<s>, </s> x2, spaces).
    4. Ask BART to fill that budget -> summary nearly fills the entire window.

    Example: claim=60 tok, explanation=100 tok -> budget = 512-160-5 = 347 tok
    -> BART generates up to 347 tokens, far more than the fixed 130 before.
    """
    claim_s = claim.strip()
    expl_s  = explanation.strip()

    # Measure tokens already consumed by claim + explanation
    base_len = len(tokenizer(
        claim_s,
        expl_s if expl_s else None,
        add_special_tokens=True,
        truncation=True,
        max_length=MAX_TOKENS,
    )["input_ids"])

    # Budget for the summary (leave 5 tokens for sep tokens + safety margin)
    summary_budget = max(SUMMARY_MIN_LEN + 1, MAX_TOKENS - base_len - 5)

    summary = summarize_text(main_text, max_new_tokens=summary_budget)
    sep     = f"{tokenizer.sep_token}{tokenizer.sep_token}"
    part_b  = f"{expl_s} {sep} {summary}" if summary else expl_s
    return claim_s, part_b
...
def _save_training_curves(history: dict, out_dir: str):
    ...
    plt.suptitle("Full Model -- Training Curves (claim + explanation + BART summary)",
                 fontsize=10, y=1.02)
    plt.tight_layout()
...
def run_ablation(train_split, val_split, test_split):
    ...
    configs = [
        ...
        ("full_bart_summary", lambda s: build_input(
